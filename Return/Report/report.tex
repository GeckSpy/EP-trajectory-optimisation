...
A track is originally a .png file which looks like the left image of figure \ref{figure:track}. Then, the image is converted to a matrix $T$ such that $T[0][0]$ is the bottom left corner. After that, we crop the image, compute the starting point, and determine the lines of the track (these will be explained in the reward section) to obtain a final result that looks like the right image of figure \ref{figure:track}. The white squares represent the road, and the green point represents the starting point.
...
The car's physics model is quite simple. It follows a 2D cartoon-like set of physical laws that behave as follows:\\
The car is characterized by two main properties: its speed $s \in [0, \text{MaxSpeed}]$ and its rotation $\alpha \in [0, 360]$.\\
The physical laws work as follows: at each time step, the car moves to its next coordinates in the direction defined by its rotation, covering a distance equal to its speed.\\
If the current coordinates of the car are $(x, y)$, its speed is $s$, and its rotation is $\alpha$, then after one time step, the new coordinates of the car will be:
\[
(s \cdot \cos\left(\frac{\pi}{180}\alpha\right) + x,\; s \cdot \sin\left(\frac{\pi}{180}\alpha\right) + y)
\]


Moreover, at each time step, the car can take an action:
\mlist{
...
We could model a state by the matrix of the track and the information of the car but this would lead to a too big state because a track can have size $900\times600$ pixels and there is a great diversity of possible tracks. 
To reduce the dimension of the problem, we decided represent a state as a real car racer would see it. Then, the state of a car is an array $V$ of size $8$.
\mlist{
