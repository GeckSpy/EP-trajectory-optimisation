...
The total reward of a car behaviour is the sum of all reward of a simulation with a car behaviour.
	...
	\part{Deep Q-learning}
		...
		\begin{description}
			...
			\item[$T$] is the transition function describing what is the next state of our agent after choicing one action : $T : S \times A \rightarrow S$
			\item[$R$] is the reward function giving   
		\end{description}
		$S$ is a set state in which our agent can be,
		$A$ is the set of actions our agent can take no matter what state it is in and $R$ a reward function 
		that gives a reward $R(a,r)$
